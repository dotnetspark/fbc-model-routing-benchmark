...
> - **Done:** `routers/` (base, custom, notdiamond_router, difficulty_baseline, routellm_router), the Docker setup, `run_router_dryrun.py`, `analysis/router_comparison.py`, the selection-gravity chart (custom + RouteLLM + difficulty heuristic), FINDINGS § Lesson 7, lessons/07 checkpoint. **Regenerate:** `docker compose run --rm routers` then `.venv311/Scripts/python analysis/router_comparison.py`. **Remaining:** add `NOT_DIAMOND_API_KEY` to `.env` to populate NotDiamond's bar, then merge `feat/lesson7_router`.
