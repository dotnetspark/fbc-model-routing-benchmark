...
>
> **Measured result.** Two *real* routers — **RouteLLM** (its local `bert` classifier; 89% cheap / 11% strong) and **NotDiamond** (its API; 42% strong / 58% cheap across a multi-provider catalog) — plus a transparent difficulty heuristic all sit **entirely in the ungrounded band**. The custom, extensible lookup-table router sits entirely in the *grounded* band (grounded-cheap clears the floor everywhere, so it never needs the strong tier). Following the custom router's picks implies **~77% correct citations vs. ~30% (RouteLLM) / ~31% (NotDiamond)** — a ~47-point gap that is entirely the grounding blind spot. Note NotDiamond *can* be trained on your own eval data, so it isn't "closed" — but even trained it optimizes *which model*, with no first-class input for whether grounding is available. The claim is **expressiveness**, not "smarter": for a verifiable-answer domain the dominant lever is *whether to ground*, which tier-routers — trainable or not — cannot express. Full synthesis + parameter provenance in [`FINDINGS.md`](../FINDINGS.md) § Lesson 7.
