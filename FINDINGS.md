...
### 4. Grounding is necessary, not sufficient — and the failure-mode split survives it
**Grounding-compliance** (cited a section actually present in the supplied passage, measured over *all* answers — abstentions count as non-compliant) is 84.1% (Haiku), 70.5% (Opus), 62.5% (SLM). That single number mixes two very different failures, so decompose it against the raw rows: grounded **Opus never cited an absent section** (0 of 31 citations) — its entire non-compliance is abstention ("Not found in context", 13/44), caution that now *costs* it accuracy the forced-citation Haiku captures. Grounded **Haiku, which cannot abstain, cited an absent section in 15.9% of answers** (7/44) — residual invention is real, and concentrated in the forced-schema tier. Grounded **phi3 mostly abstains** (14/40) and rarely invents (1 of 26 citations). The cold failure-mode taxonomy survives grounding, with the *output contract* selecting which failure remains. Treat grounding as risk-reduction, not a guarantee.
